...
AUM_DIR = "10_AUM"  # Assets Under Management lives under each owner

_slug_rx = re.compile(r"[^a-z0-9]+")
def slugify(s: Optional[str]) -> str:
    if not s: return "unknown"
    s = _slug_rx.sub("-", s.strip().lower()).strip("-")
    return s or "unknown"

def _tag(name: str, idlike: Optional[int | str]) -> str:
    return f"{slugify(name)}{'-' + str(idlike) if idlike not in (None, '', 'null') else ''}"
...
OWNER_SUBS = [
    "01_Profile","02_Agreements","03_Tax","04_Comms","05_Reports","06_Bill_Pay","07_Legal",
    AUM_DIR,
]
...
PROPERTY_SUBS = [
    "01_Units","02_Leases","03_Photos","04_Inspections","05_Work_Orders","06_Legal",
    "07_Financials","08_Acquisition_Docs","09_Notices","10_Construction","11_Media",
]
...
UNIT_SUBS = [
    ...
    "03_Inspections","04_Work_Orders","05_Legal","06_Media","07_Construction",
]

# ---------- Tenancy (application → lease → move-out) ----------
TENANCY_SUBS = [
    "00_Application","01_Screening",
    "02_Lease/Signed","02_Lease/Amendments","02_Lease/Addenda",
    "03_Correspondence","04_Notices","05_Rent","06_Subsidy","07_Maintenance",
    "08_Move_In","09_Move_Out","10_Legal","11_Compliance","12_Documents",
]
...
def tenancy_root(owner_name: str, owner_id: Optional[int | str],
    ...
    return f"{unit_root(owner_name, owner_id, property_name, property_id, unit_name, unit_id)}/02_Tenancies/{tenancy_key(meta)}"

# ---------- (legacy) property-level lease index ----------
...
LEASE_SUBS = ["Signed_Lease_Agreement","Amendments","Tenant_Correspondence"]
